# AIDEN — Catálogo de las 45 herramientas (fuente para la wiki)

> Documento-fuente para el segundo cerebro. Una ficha por herramienta (tool) de function-calling:
> qué hace, cuándo se usa, parámetros y en qué módulo vive. Estado: junio 2026, 45 tools.
...
| `contestar_llamada` | ATIENDE la llamada entrante como contestadora: ACEPTA (pulsa "Accept" vía UI Automation), le dice el mensaje al contacto con la voz de AIDEN (VB-CABLE) y CUELGA. No necesita el contacto; si tú ya la tomaste, no interviene. `mensaje` opcional (default cortés). | `mensaje?` (en 3ª persona) | Comunicacion/Llamadas.py |

> Llamadas entrantes: el **vigilante de fondo** (`Comunicacion/Vigilante_Llamadas.py`) detecta la ventana de llamada, avisa por voz y, si no reaccionas, llama a `contestar_llamada` solo. Ver *Arquitectura §10* (comportamientos de fondo).
...
## 10. Auto-programación y construcción de proyectos

| Herramienta | Qué hace / cuándo | Parámetros | Módulo |
|---|---|---|---|
| `Auto_Modificacion` | AIDEN se da a sí mismo una nueva función/habilidad: delega a Claude Code (headless, sin sandbox, repo real), valida sintaxis y la recarga en caliente. | `nombre_habilidad`, `instruccion` | Comunicacion/Funciones_Variadas.py |
| `crear_proyecto` | Construye un proyecto/app NUEVO y aparte delegando a Claude Code, en sandbox (`Proyectos_AIDEN/`) y en segundo plano; avisa por voz al terminar. | `instruccion`, `nombre?` | Sistema/Programador.py |
| `ejecutar_proyecto` | Ejecuta (corre) el código de un proyecto que AIDEN ya creó y devuelve su salida. | `nombre?`, `archivo?` | Sistema/Programador.py |

> Ver la fuente *AIDEN Constructor* para el detalle de cómo `Auto_Modificacion` y `crear_proyecto`
> comparten el mismo patrón (delegar a Claude Code) pero con sandbox vs repo real.

---

## Resumen por categoría (conteo)
- Comunicación: 4 · Control PC: 14 · Búsqueda/info: 5 · Visión/pantalla: 3 · Finanzas: 2
- Productividad: 6 · Resúmenes/utilidades: 3 · Memoria: 3 · Experto/código: 2 · Auto-prog/construcción: 3
- **Total: 45** ✅
...
y (`resumir_documento`+`resumir_youtube`→`resumir`) → 43 → sumar `crear_proyecto` + `ejecutar_proyecto`
(AIDEN Constructor) → **45**. Motivo de las fusiones: más tools degradan la precisión del
function-calling y pesan ~8k tokens por llamada. Las dos tools nuevas se aceptaron porque dan
capacidad genuinamente nueva (no son solapamiento de algo existente).

---

### Para la wiki
- Crear una **página de entidad por herramienta** (45 páginas), enlazada a su **módulo** y a los
  **conceptos** que toca (ej. `modo_gaming` ↔ *liberación de VRAM*; `consultar_experto`/`explicar_error`
  ↔ *modelo de dos niveles*; `recordar_conversacion` ↔ *las tres memorias*; `crear_proyecto`/
  `Auto_Modificacion` ↔ *AIDEN Constructor*).
- Crear una **página de decisión**: *"anti-bloat: por qué 45 y no 47 tools"*.
